...
class GoodsDetailPage extends StatefulWidget {
  ...
  GoodsDetailPage({Key key, this.goodsNo}) : super(key: key);
  _GoodsDetailPageState createState() => _GoodsDetailPageState();
}

class _GoodsDetailPageState extends State<GoodsDetailPage> {
  Model getGoodsDetailModel(BuildContext context) {
    final globalModel = GlobalModel().of(context);
    // 获取商品详情
    globalModel.getGoodsDetail(widget.goodsNo);
    // 获取租期列表
    // globalModel.getGoodsSkuDetail(widget.goodsNo);
    // 获取优惠券列表
    globalModel.getGoodsAboutCoupons(widget.goodsNo);

    print('获取商品详情model ${globalModel.detailPage}');
          ...
          ? Container(
              child: Scaffold(
                ...
                body: Stack(children: <Widget>[
                  ListView(
                    children: <Widget>[
                      headBanner(context, model.goodsDetail),
                      new Container(
                        child: detail(context, model.goodsDetail),
                      ...
                      new Container(
                          ...
                          child: ProductParam(snapshot: model.goodsDetail))
                    ],
                  )
                ]),
                bottomSheet: footer(context, model),
              ),
            )
          : Center(
  ...
  Future _getBackInfo(BuildContext context) async {
    print('啦啦啦啦我打印啦 ${widget.goodsNo}');
    var goodsDetailRes = await GoodsApiInterface.getGoodsDetail(widget.goodsNo);
      ...
      children: <Widget>[
        ...
        new Container(
            ...
            child: Column(children: <Widget>[
              ...
              SizedBox(height: 5.0),
              model.goodsLabelList.length > 0
                  ? buildLabel(context, model.goodsLabelList)
                  : Text(''),
            ]))
      ],
    );
  ...
  Widget headBanner(context, model) {
    return new Row(
  ...
  Widget initBanner(height, model) {
    print('初始化banner ${model.toString()}');
    List bannerList = model?.pictureList ?? [];
    return Container(
        color: Colors.black,
